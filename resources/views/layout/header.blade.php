                    ...
                    @else
                            ...
                            @foreach(Cart::content() as $row)
                                <div class="media">
                                    <a href="javascript:void(0)"
                                       class='remove-icon cart-popup__remove-icon remove-product-item'
                                       data-rowId="{{$row->rowId}}"><span class="glyphicon glyphicon-remove"></span></a>
                                    <img class="" src="uploaded/product/{{$row->options->image}}" alt="image"
                                         width="45px">
                                    <div class="media-body">
                                        <p class="mt-0 media__product-title"><a
                                                    href="san-pham/{{$row->id}}/{{$row->options->slug_name}}.html">{{$row->name}}</a>
                                        </p>
                                        <?php    $size = App\Size::find($row->options->size_id);
                                        $size_name = $size->name;
                                        ?>
                                        <p class='product-price product-price__popup'><span>Size: {{$size_name}}</span>
                                            | <span class=''>{{ number_format($row->price) }}</span> * {{$row->qty}} vnđ
                                        </p>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <div class='dropdown-cart__caption'>
                            ...
                            <div class='text-center dropdown-cart__bottom'>
                                <a href="{{ route('giohang')}}" class='btn__link btn__link-dropdow-cart'><span
                                            class="glyphicon glyphicon-search"></span> Chi tiết</a>
                                <a href="javascript:void(0)" class='btn__link btn__link-dropdow-cart close_popup_cart'>Tiếp
                                    tục mua hàng</a>
                            </div>
                        </div>
                        <?php
        ...
        <div class="search-box">
            <form>
                <div class="form-input-search">
                    <input type="text" name="keyword" placeholder="Nhập từ khóa cần tìm kiếm" class="search-input" value="{{request()->keyword}}">
                            ...
                            <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                @foreach($cate_child as $cate_c)
                                    <a class="dropdown-item"
                                       href="danh-muc/{{$cate_c->id}}/{{$cate_c->slug_name}}.html">{{$cate_c->name}}</a>
                                @endforeach
                            </div>
                        </li>
                    @else
                        <li class="nav-item">
                            <a class="nav-link"
                               href="danh-muc/{{$cate->id}}/{{$cate->slug_name}}.html}}">{{$cate-name}}</a>
                        </li>
                    @endif
                @endif
            @endforeach
