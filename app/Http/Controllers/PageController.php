...
class PageController extends Controller
{
    ...
    {
        $cate = Category::with('childrens')->find($id);
        if (empty($cate)) {
            return redirect(route('trang-chu'));
        }

        $cateIds = $this->pluckRecursive($cate->toArray());
        ...
        return view('page.category', compact('cate', 'products', 'brands', 'sizes'));
    }

    private function pluckRecursive($array = [], $key = 'id')
    {
        ...
        if (!empty($array['childrens'])) {
            foreach ($array['childrens'] as $item) {
                $result = array_merge($result, $this->pluckRecursive($item, 'id'));
            }
        }
    ...
    public function postCheckout(CustomerRequest $request)
    {
        ...
        if ($flag == false) {
            ...
            if ($customer->save()) {   //lưu thonong tin dơn hàng
                ...
                if ($bill->save()) {   //lưu thông tin chi tiết đơn hàng
